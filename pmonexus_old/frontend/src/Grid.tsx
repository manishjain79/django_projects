...
export function Grid(p: Props) {
  ...
  return (
    <table id="task-grid">
      <colgroup>
        {p.cols.map(c => <col key={c.key} className={c.key === p.selectedCol ? 'col-sel' : undefined} style={{ width: (p.colWidths[c.key] || c.w) + 'px' }} />)}
        ...
        <tr>
          {p.cols.map(c => (
            <th key={c.key}
              ...
              className={c.key === p.selectedCol ? 'col-sel-th' : undefined}
              style={c.key === 'name' ? { textAlign: 'left', paddingLeft: '.5rem' } : undefined}
              onClick={e => { if (!(e.target as HTMLElement).classList.contains('col-resizer')) p.onColSelect(c.key); }}
