...
from core.models import (
    ...
    TaskCustomFieldValue, TaskDependency, TaskStatus, TaskType, TimeEntry, User, WorkflowStatus,
    WorkspaceMember, WorkspaceRole,
)
from pmo.notifications import notify_assignment
from pmo.scheduler import (
    DEFAULT_WORKING_DAYS, add_working_days, count_working_days, get_calendar, is_working_day,
...
def portfolio_list(request):
    ...
    portfolios = Portfolio.objects.filter(workspace=workspace).select_related("owner").order_by("name")
    rows = []
    for p in portfolios:
        row = _portfolio_rollup(p)
        row["projects"] = [pr for pr in row["projects"] if pr.id in visible_ids]
        if row["projects"] or _is_admin(request):
            rows.append(row)
...
def _resource_off_days(workspace):
    ...
    off = {}
    for pr in ResourceProfile.objects.filter(workspace=workspace).prefetch_related("time_off"):
        days = set()
        for t in pr.time_off.all():
            d = t.start_date
            while d <= t.end_date:
                days.add(d)
                d += timedelta(days=1)
...
def _resource_working_days(workspace):
    ...
    out = {}
    for pr in ResourceProfile.objects.filter(workspace=workspace):
        if pr.working_days_json:
            out[pr.user_id] = set(pr.working_days_json)
    return out
