...
class ResourceTimeOff(models.Model):
    ...
    #         raise ValidationError(
    #             "This time-off period overlaps an existing time-off."
    #         )
...
class ProjectTemplate(models.Model):
    """A reusable project skeleton: tasks with relative working-day offsets."""
    workspace = models.ForeignKey(Workspace, on_delete=models.CASCADE, related_name="templates")
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True, null=True)
    data_json = models.JSONField(default=dict)   # {"tasks": [{i, title, parent_i, start_off, dur, milestone, preds, description}]}
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True, related_name="templates")
    created_at = models.DateTimeField(auto_now_add=True)
