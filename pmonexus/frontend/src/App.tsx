...
import { addDays, D, buildFlat, childrenOf, computeCritical, computeSlack, diffDays, iso, makeWorkday, type SortMode } from './util';
...
export interface AppConfig {
  ...
  urls: {
    ...
    projectsJson: string; projectCreate: string; exportMsp?: string; presence?: string; projectDashboard?: string;
...
const BASE_COLS: ColDef[] = [
  ...
  { key: 'total_slack', label: 'Total Slack', w: 92 },
  { key: 'free_slack', label: 'Free Slack', w: 88 },
  { key: 'baseline_start', label: 'Baseline Start', w: 110 },
  { key: 'baseline_end', label: 'Baseline Finish', w: 110 },
];
const DEFAULT_HIDDEN = ['sprint', 'points', 'description', 'deadline', 'work', 'total_slack', 'free_slack', 'baseline_start', 'baseline_end'];
...
const TABS = ['project', 'task', 'view', 'format', 'agile', 'report', 'templates', 'team', 'window'] as const;
...
export function App({ cfg }: { cfg: AppConfig }) {
  ...
  const [progressLine, setProgressLine] = useState(() => localStorage.getItem('pmo-progressline') === '1');
  const [groupBy, setGroupBy] = useState<'none' | 'resource' | 'status' | 'phase'>('none');
  ...
  const [tplList, setTplList] = useState<{ id: number; name: string; description: string; tasks: number; created: string; by: string }[] | null>(null);
  ...
  const [selectedCols, setSelectedCols] = useState<Set<string>>(new Set());  // Ctrl+click adds more columns
  ...
  const displayFlatRef = useRef<FlatRow[]>([]);
  ...
  useEffect(() => { localStorage.setItem('pmo-progressline', progressLine ? '1' : '0'); }, [progressLine]);
  ...
  /** MS Project block indent: the whole selection becomes children of the task above it. */
  const indentMany = useCallback((ids: number[]) => {
    if (!data || !ids.length) return;
    const rows = flat.filter(r => ids.includes(r.t.id)).sort((a, b) => a.row - b.row);
    if (!rows.length) return;
    const first = rows[0];
    const above = first.row > 0 ? flat[first.row - 1].t : null;
    if (!above || ids.includes(above.id)) { store.say('Nothing above to indent under', true); return; }
    setCollapsed(prev => { const n = new Set(prev); n.delete(above.id); return n; });
    const chosen = new Set(ids);
    reorderFromTasks(data.tasks.map(t => (chosen.has(t.id) ? { ...t, parent_id: above.id } : t)));
  }, [data, flat, store, reorderFromTasks]);

  /** Block outdent: each selected task moves to its grandparent (children of selected move along). */
  const outdentMany = useCallback((ids: number[]) => {
    if (!data || !ids.length) return;
    const chosen = new Set(ids);
    let changed = false;
    const tasks = data.tasks.map(t => {
      if (!chosen.has(t.id) || t.parent_id === null) return t;
      const parent = data.tasks.find(x => x.id === t.parent_id);
      if (parent && chosen.has(parent.id)) return t;   // moves with its selected parent
      changed = true;
      return { ...t, parent_id: parent ? parent.parent_id : null };
    });
    if (!changed) { store.say('Already at the top level', true); return; }
    ...
    const h = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && !typing) {
        if (e.key.toLowerCase() === 'z' && !e.shiftKey) { e.preventDefault(); store.undo(); }
          ...
          e.preventDefault();
          const rows = displayFlatRef.current;
          setSelectedIds(new Set(rows.map(r => r.t.id)));
          if (rows[0]) setPrimaryId(rows[0].t.id);
        }
      }
  ...
  const slackMap = computeSlack(data.tasks, data.calendar);
  ...
  const displayFlat: FlatRow[] = (filterRowOn && activeColFilters.length)
    ...
    : viewFlat;
  displayFlatRef.current = displayFlat;

  // Formatting targets: explicit row selection wins; otherwise a selected
  // column applies the format down the whole column (MS Project behaviour).
  const fmtIds = selIdList.length ? selIdList
    : (selectedCols.size || selectedCol ? displayFlat.map(r => r.t.id) : []);
  const fmtAny = fmtIds.length > 0;
  ...
  const costView = (() => {
    ...
    const CV = EV - AC, SV = EV - PV;                       // cost / schedule variance ($)
    const EAC = CPI ? BAC / CPI : BAC;                      // forecast at completion (current CPI trend)
    const ETC = Math.max(0, EAC - AC);                     // estimate to complete
    const VAC = BAC - EAC;                                 // + = under budget, − = over budget
    const overPct = BAC ? (EAC - BAC) / BAC * 100 : 0;     // budget-vs-actual trend
    return { rows, BAC, EV, AC, PV, CPI, SPI, CV, SV, EAC, ETC, VAC, overPct };
  })();
  ...
  return (
    <div className="planner">
      <div id="ribbon">
            ...
            <div className="rgroup">
              <div className="rgroup-body rgroup-stack">
                ...
                <div className="rrow">
                  ...
                  <Sm icon={Outdent} disabled={!any} title="Outdent (Shift+Tab)" onClick={() => outdentMany(selIdList)} />
                  <Sm icon={Indent} disabled={!any} title="Indent — make subtask (Tab)" onClick={() => indentMany(selIdList)} />
        ...
        {!fileOpen && tab === 'format' && (
          <div className="ribbon-pane">
            <div className="rgroup">
              <div className="rgroup-body rgroup-stack">
                <div className="rrow">
                  <select className="rb-select" disabled={!fmtAny} value={primary?.t.format?.family || ''}
                    onChange={e => store.applyUpdate(fmtIds, { format: { family: e.target.value || null } })}>
                  ...
                  <select className="rb-select" style={{ width: 58 }} disabled={!fmtAny} value={primary?.t.format?.size || ''}
                    onChange={e => store.applyUpdate(fmtIds, { format: { size: e.target.value || null } })}>
                    <option value="">Size</option>
                    {[8, 9, 10, 11, 12, 13, 14, 16, 18].map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <Sm icon={Bold} disabled={!fmtAny} on={!!primary?.t.format?.bold} title="Bold"
                    onClick={() => primary && store.applyUpdate(fmtIds, { format: { bold: !primary.t.format?.bold } })} />
                  <Sm icon={Italic} disabled={!fmtAny} on={!!primary?.t.format?.italic} title="Italic"
                    onClick={() => primary && store.applyUpdate(fmtIds, { format: { italic: !primary.t.format?.italic } })} />
                </div>
                <div className="rrow swatch-row">
                  <span className="swatch-tag">A</span>
                  <span className="swatch none" title="No colour" onClick={() => fmtAny && store.applyUpdate(fmtIds, { format: { color: null } })} />
                  {COLORS.map(c => <span key={c} className="swatch" style={{ background: c }}
                    onClick={() => fmtAny && store.applyUpdate(fmtIds, { format: { color: c } })} />)}
                  <input type="color" className="color-pick" title="Custom text colour" disabled={!fmtAny}
                    onChange={e => store.applyUpdate(fmtIds, { format: { color: e.target.value } })} />
                  <span className="swatch-sep" />
                  <span className="swatch-tag">▉</span>
                  <span className="swatch none" title="No highlight" onClick={() => fmtAny && store.applyUpdate(fmtIds, { format: { bg: null } })} />
                  {BGS.map(c => <span key={c} className="swatch" style={{ background: c }}
                    onClick={() => fmtAny && store.applyUpdate(fmtIds, { format: { bg: c } })} />)}
                  <input type="color" className="color-pick" title="Custom highlight colour" disabled={!fmtAny}
                    onChange={e => store.applyUpdate(fmtIds, { format: { bg: e.target.value } })} />
                </div>
              </div>
            ...
            <div className="rgroup">
              <div className="rgroup-body rgroup-stack">
                ...
                <div className="rrow">
                  ...
                  <Sm label="Top" disabled={!fmtAny} on={primary?.t.format?.valign === 'top'} title="Align selected row(s) to top" onClick={() => store.applyUpdate(fmtIds, { format: { valign: 'top' } })} />
                  <Sm label="Middle" disabled={!fmtAny} on={primary?.t.format?.valign === 'middle'} onClick={() => store.applyUpdate(fmtIds, { format: { valign: 'middle' } })} />
                  <Sm label="Bottom" disabled={!fmtAny} on={primary?.t.format?.valign === 'bottom'} onClick={() => store.applyUpdate(fmtIds, { format: { valign: 'bottom' } })} />
                </div>
              </div>
            ...
            <div className="rgroup">
              <div className="rgroup-body">
                ...
                <Big icon={Activity} label="Progress Line" on={progressLine} title="Draw the status-date progress line (zigzags left for behind-schedule tasks)" onClick={() => setProgressLine(v => !v)} />
            ...
            <div className="rgroup">
              <div className="rgroup-body rgroup-stack">
                <div className="rrow">
                  <span className="rb-tag">Group by</span>
                  <select className="rb-select" value={groupBy} title="Band the sheet by a field (replaces the outline while active)"
                    onChange={e => setGroupBy(e.target.value as typeof groupBy)}>
                    <option value="none">No grouping</option>
                    <option value="resource">Assigned resource</option>
                    <option value="status">Status</option>
                    <option value="phase">Phase (top summary)</option>
                  </select>
                </div>
                {groupBy !== 'none' && <div className="rrow"><span className="rb-hint">Outline is replaced by group bands while grouping is on.</span></div>}
              </div>
              <div className="rgroup-label">Group</div>
            </div>
          </div>
        )}
          ...
          <div className="ribbon-pane">
            <div className="rgroup">
              <div className="rgroup-body">
                ...
                <BigLink icon={BarChart3} label="Project Dashboard" href={cfg.urls.projectDashboard || cfg.urls.dashboard} title="Steerco-level dashboard for this project" />
                {cfg.urls.portfolioTimeline && <BigLink icon={GanttChart} label="Portfolio Timeline" href={cfg.urls.portfolioTimeline} title="All projects as bars on one shared Gantt axis" />}
        ...
        {!fileOpen && tab === 'templates' && (
          <div className="ribbon-pane">
            <div className="rgroup">
              <div className="rgroup-body">
                <Big icon={LayoutTemplate} label="Save as Template" title="Store this plan's structure as a reusable template"
                  onClick={async () => {
                    const name = prompt('Template name:', data.project.name + ' template');
                    if (!name?.trim()) return;
                    const desc = prompt('Short description (optional):') || '';
                    try {
                      const res = await fetch(`/templates/save-from/${cfg.projectId}/`, {
                        method: 'POST',
                      ...
                      const d = await res.json();
                      store.say(`Template "${name.trim()}" saved (${d.tasks} tasks)`);
                      setTplList(null);
                    } catch { store.say('Could not save the template', true); }
                  }} />
                <Big icon={FolderOpen} label="Browse Templates" title="Use or manage saved templates"
                  onClick={() => {
                    setTplOpen(true);
                    fetch('/templates/json/').then(r => r.json()).then(d => setTplList(d.templates)).catch(() => setTplList([]));
                  }} />
              </div>
              <div className="rgroup-label">Project Templates</div>
            </div>
            <div className="rgroup">
              <div className="rgroup-body" style={{ maxWidth: 300 }}>
                <p className="rgroup-note">
                  Templates store the task structure with relative working-day offsets, dependencies and milestones.
                  New projects can start from a template, an Excel list or an MS Project file — see the New Project dialog or Project → Import.
                </p>
              </div>
              <div className="rgroup-label">About</div>
            </div>
          </div>
        )}
      ...
      {!fileOpen && viewMode === 'cost' && (
        <div className="editor-card">
          <div className="baseline-view">
            ...
            <div className={'evm-banner ' + (costView.overPct > 1 ? 'over' : costView.overPct < -1 ? 'under' : 'ontrack')}>
              {costView.overPct > 1
                ? <span>Forecast <b>{Math.abs(Math.round(costView.overPct))}% over budget</b> — EAC {money(costView.EAC)} vs BAC {money(costView.BAC)} (VAC {money(costView.VAC)})</span>
                : costView.overPct < -1
                  ? <span>Forecast <b>{Math.abs(Math.round(costView.overPct))}% under budget</b> — EAC {money(costView.EAC)} vs BAC {money(costView.BAC)}</span>
                  : <span>On budget — EAC {money(costView.EAC)} ≈ BAC {money(costView.BAC)}</span>}
            </div>
            <div className="evm-cards">
              ...
              <div className="evm-card"><label>Cost var (CV)</label><span className={costView.CV < 0 ? 'bad' : 'good'}>{money(costView.CV)}</span></div>
              <div className="evm-card"><label>Sched var (SV)</label><span className={costView.SV < 0 ? 'bad' : 'good'}>{money(costView.SV)}</span></div>
              <div className="evm-card"><label>Forecast (EAC)</label><span>{money(costView.EAC)}</span></div>
              <div className="evm-card"><label>To complete (ETC)</label><span>{money(costView.ETC)}</span></div>
              <div className="evm-card"><label>Var at compl. (VAC)</label><span className={costView.VAC < 0 ? 'bad' : 'good'}>{money(costView.VAC)}</span></div>
            </div>
            <div className="bl-scroll">
              <table className="bl-table">
            ...
            {viewMode !== 'chart' && (
              <div className={viewMode === 'sheet' ? 'grid-pane full' : 'grid-pane'}
                ...
                <Grid store={store} flat={displayFlat} cols={cols} colWidths={colWidths}
                  ...
                  statuses={statusDefs} members={data.members} slack={slackMap} groupBy={groupBy}
                  ...
                  selectedCols={selectedCols.size ? selectedCols : (selectedCol ? new Set([selectedCol]) : new Set())}
                  onColSelect={(k, e) => {
                    // MS Project semantics: header click selects the COLUMN only
                    // (light tint); Ctrl+click adds more columns. Rows stay as-is.
                    setSelectedCol(k); setAlignCol(k);
                    setSelectedCols(prev => {
                      if (e.ctrlKey || e.metaKey) { const n = new Set(prev); n.has(k) ? n.delete(k) : n.add(k); return n; }
                      return new Set([k]);
                    });
                  }}
            ...
            {viewMode !== 'sheet' && (
              <div className="gantt-pane" ref={ganttScrollRef}>
                <Gantt store={store} flat={displayFlat} dayWidth={dayWidth} showCritical={showCritical}
                  ...
                  statusColorOf={statusColorOf} progressLine={progressLine} />
              </div>
            )}
          </div>
          {inspectorOn && (
      ...
      {tplOpen && (
        <div className="modal-bg open" onClick={e => { if (e.target === e.currentTarget) setTplOpen(false); }}>
          <div className="modal">
            <h2>Project templates</h2>
            {tplList === null && <p className="dw-muted">Loading…</p>}
            {tplList !== null && tplList.length === 0 && <p className="dw-muted">No templates yet — use "Save as Template" on any plan.</p>}
            {tplList !== null && tplList.map(t => (
              <div key={t.id} className="dw-chip" style={{ marginTop: 6 }}>
                <span><b>{t.name}</b> <span style={{ color: '#94a3b8' }}>· {t.tasks} tasks · {t.created}{t.by ? ' · ' + t.by : ''}</span></span>
                <span style={{ display: 'flex', gap: 6 }}>
                  <button className="dw-btn" style={{ padding: '2px 8px' }} onClick={async () => {
                    const name = prompt('New project name:', t.name.replace(/ template$/i, ''));
                    if (!name?.trim()) return;
                    const res = await fetch(cfg.urls.projectCreate, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json', 'X-CSRFToken': (document.cookie.match('(^|;)\\s*csrftoken\\s*=\\s*([^;]+)') || []).pop() || '' },
                      body: JSON.stringify({ name: name.trim(), template_id: t.id }),
                    });
                    if (!res.ok) { store.say(await res.text(), true); return; }
                    const d = await res.json();
                  ...
                  <button onClick={async () => {
                    ...
                    await fetch(`/templates/${t.id}/delete/`, {
                      method: 'POST',
                      headers: { 'X-CSRFToken': (document.cookie.match('(^|;)\\s*csrftoken\\s*=\\s*([^;]+)') || []).pop() || '' },
                    });
                    setTplList(prev => prev ? prev.filter(x => x.id !== t.id) : prev);
                  }}>✕</button>
                </span>
              </div>
            ))}
            <div className="modal-actions">
              <button className="dw-btn primary" onClick={() => setTplOpen(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
