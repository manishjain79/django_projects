...
def next_working_day(d, cal):
    MAX_LOOKAHEAD_CALENDAR_SEARCH_DAYS = 731
    for _ in range(MAX_LOOKAHEAD_CALENDAR_SEARCH_DAYS):
...
def shift_working_days(start_date, offset, cal):
    """
    Move `offset` working days from `start_date`.

    Positive offset  -> move forward.
    Negative offset  -> move backward.

    If `start_date` falls on a non-working day, it is first snapped to the
    nearest working day in the direction of travel.
    """
    moving_forward = offset >= 0
    step = 1 if moving_forward else -1

    current_date = (
        next_working_day(start_date, cal)
        if moving_forward
        else prev_working_day(start_date, cal)
    )

    for _ in range(abs(offset)):
        current_date += timedelta(days=step)
        current_date = (
            next_working_day(current_date, cal)
            if moving_forward
            else prev_working_day(current_date, cal)
        )
...
def count_working_days(start_date, end_date, cal):
    ...
    while current_date <= end_date:
        if is_working_day(current_date, cal):
            working_days += 1
