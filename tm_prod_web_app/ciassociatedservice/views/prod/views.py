...
associatedservice_file = 'TM_CIs_Associated_Service.xlsx'
cmdb_file = 'TM_GCs_CMDB.xlsx'
...
def datafile(app_url):
    print(app_url)
    if 'WEBSITE_HOSTNAME' in os.environ:
        DATAFILE_DIR = Path('/files')
        associatedservice_filepath = DATAFILE_DIR / associatedservice_file
        cmdb_filepath = DATAFILE_DIR / cmdb_file
    else:
        DATAFILE_DIR = Path(__file__).resolve().parent.parent.parent/'appfiles'
        associatedservice_filepath = DATAFILE_DIR / associatedservice_file
        cmdb_filepath = DATAFILE_DIR / cmdb_file
    if 'associatedservice' in app_url:
        return associatedservice_filepath
    if 'cmdb' in app_url:
        return cmdb_filepath
...
def gclist(request):
    ...
    filename = datafile(app_url)
    filename = os.path.basename(filename)
    print(f'My url is: {app_url}')
    # (name, gclist, rolepermission) = parse_header_test(request)    
    (name, gclist, rolepermission) = parse_header(request)
    context = {
        'name': name,
    ...
    if rolepermission == 'Admin':
        if request.method == 'POST':
            ...
            if uploaded_file_name != filename:
                context['filename_error'] = f'File name must be {filename}'
...
def assetlist(request, gcname):
    ...
    if gcname in gclist:
        ...
        if 'cmdb' in app_url:
            xls_sheetname = f'{gcname}_CMDB'
        context = {
            # 'data_table': gethtmltemplate(xls_filename,'CIs_Asscoiated Service Offering','GC', gcname),
            'data_table': gethtmltemplate(filename,xls_sheetname,'GC', gcname),
